function [z, max_data] = allZs( spikes, stimtime, spontime, fig_handle, x, y )
...
min_wind_width = 0.010; % ms
max_wind_width = 2;

wind_step = 0.050;
wind_bins = min_wind_width:wind_step:max_wind_width;

z.Zs = zeros( length(wind_bins), length(time_bins) );
    z.Xs = time_bins;
    z.Ys = wind_bins;
...
for ww = 2:length( wind_bins )
   ...
   z.Zs(ww,:) = [z.Zs(ww,end-shift_size+1:end) z.Zs(ww,1:end-shift_size)];
end % for each Z-window size
...
% find indices of maximum Z-score
    ...
    max_data(3) = z.Xs(max_data(3));
    max_data(2) = z.Ys(max_data(2));
