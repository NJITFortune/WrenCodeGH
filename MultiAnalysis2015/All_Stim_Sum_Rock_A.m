...
endlength = min([length(fem1(1).srast.spers) length(fem2(1).srast.spers) length(fem3(1).srast.spers) length(fem4(1).srast.spers)]);
    ...
    figure(j); clf;
        ax(1) = subplot(3,1,1); 
         ...
         ax(2) = subplot(3,1,2); 
            ...
            plot(mal4(j).srast.tim(1:endlength), mal4(j).srast.spers(1:endlength), 'k');
            text(-5, 50, 'Male');
            hold off; 
...
for k = 1:length(fem1);
    totalfem{k} = fem1(k).srast.spers(1:endlength) + fem2(k).srast.spers(1:endlength) + fem3(k).srast.spers(1:endlength) + fem4(k).srast.spers(1:endlength);
    totalmal{k} = mal1(k).srast.spers(1:endlength) + mal2(k).srast.spers(1:endlength) + mal3(k).srast.spers(1:endlength) + mal4(k).srast.spers(1:endlength);
end
