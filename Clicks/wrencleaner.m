...
%% Loop through each syllable and check and corret the most common errors
for i = 1:length(in)
    ...
    if ismember(0,[in(i).trace_freq]) == 1
        ...
        if length(zips) == 1 
            ...
            figure(1); hold off; plot(in(i).trace_tim, in(i).trace_freq,'*-');
