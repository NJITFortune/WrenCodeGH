...
% Plot
    figure(1); clf; hold on;
        ...
        plot(0.9, mean(mChron), 'b.', 'MarkerSize', 16);
            errorbar(0.9, mean(mChron), std(mChron), 'b' );% /sqrt(length(mChron)));
        plot(1.9, mean(fChron), 'm.', 'MarkerSize', 16);
            errorbar(1.9, mean(fChron), std(fChron), 'm' );% /sqrt(length(fChron)));
        plot(2.9, mean(mAcute), 'b.', 'MarkerSize', 16);
            errorbar(2.9, mean(mAcute), std(mAcute), 'b' );% /sqrt(length(mAcute)));
        plot(3.9, mean(fAcute(1:end-2)), 'm.', 'MarkerSize', 16);
            errorbar(3.9, mean(fAcute(1:end-2)), std(fAcute(1:end-2)), 'm' );% /sqrt(length(fAcute(1:end-1))));
