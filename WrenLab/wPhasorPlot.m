...
for mdat = 1:2:ldat
    ...
    [mcSPER(:,ceil(mdat/2)), foo.mctim, foo.mcFs] = swPSTH(data(mdat).AH.cPhaseSpikes, Pwin, Pbinwid);
    [muSPER(:,ceil(mdat/2)), foo.mutim, foo.muFs] = swPSTH(data(mdat).AH.uPhaseSpikes, Pwin, Pbinwid);
    % Normalize the data
...
for fdat = 2:2:ldat
    % Get the data in Spikes per Second
    [fcSPER(:,ceil(fdat/2)), foo.fctim, foo.fcFs] = swPSTH(data(fdat).HA.cPhaseSpikes, Pwin, Pbinwid);
    [fuSPER(:,ceil(fdat/2)), foo.futim, foo.fuFs] = swPSTH(data(fdat).HA.uPhaseSpikes, Pwin, Pbinwid);
    % Normalize the data
