...
% Default window width for histogram if user didn't specify window
    ...
    widow = 0.300; % Time before and after transition. 300 msec looks pretty good
...
linkaxes(axc, 'xy'); figure(1); subplot(221);  ylim([-8 60]); xlim([-widow-0.01, widow+0.01]);
linkaxes(axu, 'xy'); figure(1); subplot(223);  ylim([0 55]); xlim([-widow-0.01, widow+0.01]);
...
linkaxes(fmc, 'xy'); figure(7); subplot(221); ylim([-8 60]);
