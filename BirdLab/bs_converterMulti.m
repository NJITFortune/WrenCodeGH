function zf = bs_converterMulti(spiketimes, kbd, stim, limits)
...
% Version 8 October 2016 - added MULTI functionality
...
%% Prep work %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if isstruct(spiketimes) 
            ...
            spikeCodes = spiketimes.codes(:,1);
    else
    		spikeTimes = spiketimes; % a list of spike times
            spikeCodes = 0;
    end;
    
    zf(1).allspiketimes = spikeTimes; % Add this to the output structure
...
% or a list of spiketimes. I expect that it will usually be the Spike2 structure.
    ...
    for j=1:length(Starts)
        spiketimesidx = spikeTimes > (Starts(j) - pretrig) & spikeTimes < (Starts(j) + posttrig);
        %         spiketimes = find(spikeTimes > (Starts(j) - pretrig) & spikeTimes < (Starts(j) + posttrig));
        zf(i).spikes{j} = spikeTimes(spiketimesidx) - Starts(j);
        %        zf(i).spikes{j} = spikeTimes(spiketimes) - Starts(j);
    end

    % This has been modified to handle multiple single units from Spike2
    if length(unique(spikeCodes)) > 1
        codeSpikes = unique(spikeCodes);

        for cc = 1:length(codeSpikes)
            zf(i).multi(cc).spikes = cell(length(Starts),1);
            onlyourspikes = spikeTimes(spikeCodes == codeSpikes(cc));
            
            for j=1:length(Starts)
                spikeidxs = onlyourspikes > (Starts(j) - pretrig) & onlyourspikes < (Starts(j) + posttrig);
                %         spiketimes = find(spikeTimes > (Starts(j) - pretrig) & spikeTimes < (Starts(j) + posttrig));
                zf(i).multi(cc).spikes{j} = onlyourspikes(spikeidxs) - Starts(j);
                %        zf(i).spikes{j} = spikeTimes(spiketimes) - Starts(j);
            end;
        end;
...
end; % End of loop for each stimulus
