function zf = f_converter(spiketimes, kbd, stim, limits, stimtype)
% function zf = bs_converter(SpikeTimes, KeyCodes, Stimulus, limits, Stimtype)
% F_CONVERTER Converts a Spike2 matlab file into a cell array for use
...
for i = 1:stimnum;
    ...
    if stimtype == 0; % Moving object stimulus
        % Extract velocity from the position
        zf(i).vel = medfilt1(diff(stimulus(1:100:end)), 5);
            zf(i).vel(end+1) = zf(i).vel(end);
        % Extract acceleration from the velocity
        zf(i).acc = medfilt1(diff(zf(i).vel), 5);
            zf(i).acc(end+1) = zf(i).acc(end);    
        zf(i).vatim = zf(i).tim(1:100:end);
            zf(i).vatim(end+1) = zf(i).vatim(end);
        zf(i).vastim = zf(i).stim(1:100:end);
            zf(i).vastim(end+1) = zf(i).vastim(end);
    end;
